...
def get_user_comment_list_detail(user_id):
    return [Comment.objects.get(id=x).to_dic_id() for x in get_user_comment_list_simple(user_id)]
...
def get_user_complain_list_detail(user_id):
    return [Complain.objects.get(id=x).to_dic_id() for x in get_music_complain_list_simple(user_id)]
...
def get_music_comment_list_detail(music_id):
    return [Comment.objects.get(id=x).to_dic_id() for x in get_music_comment_list_simple(music_id)]
...
def get_music_complain_list_detail(music_id):
    return [Complain.objects.get(id=x).to_dic_id() for x in get_music_complain_list_simple(music_id)]
...
def list_music_complain(request):
    if request.method == 'GET':  # 判断请求方式是否为 POST（要求POST方式）
        ...
        complains = [x.to_dic_id() for x in complains]
...
def list_music_comment(request):
    if request.method == 'GET':  # 判断请求方式是否为 POST（要求POST方式）
        ...
        comments = [x.to_dic_id() for x in comments]
